...
uint32_t setLPIConfigTableAddr(uint32_t rd, uint64_t addr, uint64_t attributes, uint32_t IDbits)
{
  ...
  #ifdef DEBUG_GIC
  LOG_DEBUG("setLPIConfigTableAddr:: Installing LPI Configuration Table on RD%d\n", rd);
  LOG_DEBUG("setLPIConfigTableAddr:: Tabble base 0x%lx, with %d ID bits\n", addr, IDbits);
  #endif
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("setLPIConfigTableAddr:: ERROR - Invalid number of ID bits\n");
    #endif
    return 1;
  }
...
uint32_t setLPIPendingTableAddr(uint32_t rd, uint64_t addr, uint64_t attributes, uint32_t IDbits)
{
  ...
  #ifdef DEBUG_GIC
  LOG_DEBUG("setLPIPendingTableAddr:: Installing LPI Pending Table on RD%d\n", rd);
  LOG_DEBUG("setLPIPendingTableAddr:: Tabble base 0x%lx, with %d ID bits\n", addr, IDbits);
  #endif
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("setLPIPendingTableAddr:: ERROR - Invalid number of ID bits\n");
    #endif
    return 1;
  }
...
{
  #ifdef DEBUG_GIC
  LOG_DEBUG("enableLPIs:: Enabling physical LPIs on RD%d\n", rd);
  #endif
...
uint32_t configureLPI(uint32_t rd, uint32_t ID, uint32_t enable, uint32_t priority)
{
  ...
  #ifdef DEBUG_GIC
  LOG_DEBUG("configureLPI:: Configuring INTID %d, with priority 0x%x and enable 0x%x, on RD%d\n", ID, priority, enable, rd);
  #endif
  ...
  if ((ID < 8192) || (ID > max_lpi))
  {
    ...
    #ifdef DEBUG_GIC
    LOG_DEBUG("configureLPI:: ERROR - INTID %d is beyond supported maximum INTID %d\n", ID, max_lpi);
    #endif
    return 1;
  }
  ...
  if (ID > max_lpi)
  {
    ...
    #ifdef DEBUG_GIC
    LOG_DEBUG("configureLPI:: ERROR - INTID %d is beyond configured maximum INTID %d\n", ID, max_lpi);
    #endif
    return 1;
  }
...
uint32_t initITSCommandQueue(uint64_t addr, uint64_t attributes, uint32_t num_pages)
{
  ...
  #ifdef DEBUG_GIC
  LOG_DEBUG("initITSCommandQueue:: Setting up Command Queue at 0x%lx, with %d pages\n", addr, num_pages);
  #endif
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("initITSCommandQueue:: ERROR - Page count for Command Queue cannot be 0\n");
    #endif
    return 1;
  }
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("initITSCommandQueue:: ERROR - Page count for Command Queue cannot be 256\n");
    #endif
    return 1;
  }
...
uint32_t setITSTableAddr(uint32_t index, uint64_t addr, uint64_t attributes, uint32_t page_size, uint32_t num_pages)
{
  ...
  #ifdef DEBUG_GIC
  LOG_DEBUG("setITSTableAddr:: Setting up ITS table %d at 0x%lx, with %d pages\n", index, addr, num_pages);
  #endif
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("initITSCommandQueue:: ERROR - Invalid index vales\n");
    #endif
    return 1;
  }
  ...
  {
    #ifdef DEBUG_GIC
    LOG_DEBUG("initITSCommandQueue:: ERROR - Invalid number of pages\n");
    #endif
    return 1;
  }
...
void disableITS(void)
{
  ...
  while((gic_its->GITS_CTLR & (1ULL << 31)) == 0)
  {}
